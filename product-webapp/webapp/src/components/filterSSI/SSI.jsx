...
const SSI = ({ instructionId, tradeDate, status, counterPartyName }) => {
  ...
  const handleDownload = async (format, instructionId) => {
    try {
      ...
      const response = await axios.post(endpoint, instructionId, {
  ...
  const handleEdit = async () => {
    try {
      ...
      localStorage.setItem("ssi", JSON.stringify(response.data));
      window.location.replace("/userHome/updateSSI");
    } catch (error) {
  ...
  const handleDelete = () => {
    const response = axios.delete(
      `http://localhost:9092/api/v1/ssi/delete/${instructionId}`
    );
